...
| **Public routes ship no Supabase key** | `NEXT_PUBLIC_SUPABASE_ANON_KEY` is inlined into any client chunk reaching `lib/supabase/browser.ts`, so a publicly reachable route with a client‑side Supabase import publishes a database credential. Public pages therefore read server‑side and pass results — and `isAdmin`, via `isAdminRequest()` — down as props. RLS is the second layer: tables not part of the published story (bodyweight, sleep) are `authenticated`‑only, so the key alone would not reach them (#345). |
